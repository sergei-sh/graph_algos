...
def prims_mst(dist_mx):
    ...
    # INF will discard dist_mx value in matrix el.w. product
    in_mst_mask = np.full(dist_mx.shape, INF)

    root = True
    while out_of_mst:
        #take minimal, move to resulting tree
        cur_vx = heappop(out_of_mst)
        vx = cur_vx.vx

        #root should be the first
        if not root:
            vx_from = cur_vx.vx_from
            # 1 will retain dist_mx value in matrix el.w. product
    ...
    return np.multiply(in_mst_mask, dist_mx)
